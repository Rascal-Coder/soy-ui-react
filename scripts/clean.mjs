...
// 简单实现控制台日志
const logger = {
  error: (...args) => console.log(COLORS.red + UNICODE.FAILURE, args.join(' ') + COLORS.reset),
  info: (...args) => console.log(COLORS.blue + args.join(' ') + COLORS.reset),
  log: console.log,
  success: (...args) => console.log(COLORS.green + UNICODE.SUCCESS, args.join(' ') + COLORS.reset)
};
 ...
 * @param {string[]} options.targets - 目标文件/目录
 * @param {object} options.stats - 统计信息
 * @param {boolean} options.verbose - 是否显示详细日志
 */
async function cleanTargetsRecursively(currentDir, options) {
  const { targets, stats, verbose } = options;
      ...
      items.map(async item => {
        try {
          ...
          if (targets.includes(item)) {
            ...
            if (verbose) {
              logger.success(`已删除: ${itemPath}`);
            }
            return;
          }

          if (stat.isDirectory()) {
            await cleanTargetsRecursively(itemPath, options);
          }
        } catch (error) {
          stats.errorCount += 1;
